...
if [ ${UNAL} != "NA" ]; then
  bowtie2 --seed ${SEED} --un-gz ${UNAL} --no-unal --score-min L,0,-0.4 -p ${NPRO} -k ${K} --very-sensitive -x ${BI} -U ${FASQ} 2>> ${FLDG}StarDust.log | check_sam ${FOLDOUT} | check_mult ${FOLDOUT} ${K} > ${FOLDOUT}multiple.SAM
else
  if [ ${FASQ: -5} == "fasta"  ]
  then
    bowtie2 --seed ${SEED} -f --no-unal --score-min L,0,-0.4 -p ${NPRO} -k ${K} --very-sensitive -x ${BI} -U ${FASQ} 2>> ${FLDG}StarDust.log | check_sam ${FOLDOUT} | check_mult ${FOLDOUT} ${K} > ${FOLDOUT}multiple.SAM
  else
    hisat2 --seed ${SEED} --no-unal --score-min L,0,-0.4 -p ${NPRO} -k ${K} --very-sensitive -x ${BI} -U ${FASQ} 2>> ${FLDG}StarDust.log | check_sam ${FOLDOUT} | check_mult ${FOLDOUT} ${K} > ${FOLDOUT}multiple.SAM
  fi
fi
