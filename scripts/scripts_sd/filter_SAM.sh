...
printf "Unique mapped reads:\t" >> ${SUMMARY_T}
tail -n 1 ${FOLDER}mult_readcount.txt >> ${SUMMARY_T}
printf "Multiple mapped reads:\t" >> ${SUMMARY_T}
head -n 2 ${FOLDER}mult_readcount.txt | tail -n 1 >> ${SUMMARY_T}
printf 'Reads with max mapping (k= %i):\t' $MAXALL>> ${SUMMARY_T}
...
then
 ...
 printf "Reads unique mapped to rtRNA and chrM:" >> ${SUMMARY_T}
...
printf "Reads unique mapped to exons:\t" >> ${SUMMARY_T}
echo $(samtools stats ${FOLDER}exons_sorted.BAM | grep -P '^SN\traw total' | cut -f 3) >> ${SUMMARY_T}
printf "Reads multiple mapped eliminated:\t" >> ${SUMMARY_T}
echo $(wc -l ${FOLDER}trashreads.txt| cut -f 1 -d " ") >> ${SUMMARY_T}
printf "Reads unique mapped left:\t" >> ${SUMMARY_T}
...
perl -e '{open(RE1,"$ARGV[0]"); while($l=<RE1>){chomp $l;$re{$l}=1;}close(RE1);
 ...
 }' ${FOLDER}trashreads.txt ${FOLDER}multiple_noexons_nortRNAM.SAM ${FOLDER}reads_passed.txt >${FOLDER}DONE_multiple.SAM

rm ${FOLDER}trashreads.txt ${FOLDER}multiple_noexons_nortRNAM.SAM

printf "Multiple mapped reads left:\n" >>  ${SUMMARY_T}
cut -f 2 ${FOLDER}reads_passed.txt | sort -n --parallel ${NUMPR} | uniq -c >> ${SUMMARY_T}

samtools view -@ ${NUMPR} -b ${FOLDER}DONE_multiple.SAM -t ${FOLDER}header_mod.txt > ${FOLDER}DONE_multiple.BAM
mv ${FOLDER}uniq_noexons_nortRNAM.BAM ${FOLDER}DONE_uniq.BAM

rm ${FOLDER}header_mod.txt  ${FOLDER}DONE_multiple.SAM  ${FOLDER}reads_passed.txt

samtools view -@ ${NUMPR} -F 256 -b -h ${FOLDER}DONE_multiple.BAM > ${FOLDER}multiple.BAM
