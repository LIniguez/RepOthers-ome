...
cat ${FOLDER}/${ROUND}_modified2.sam ${FOLDER}/${ROUND}_flag2562.sam | awk '{b=$1","$3","$4; if (!(b in a)){a[b] = $0;} } END { for (i in a) print a[i]}'| sort -T ${FOLDER} --parallel ${NUMPROC} -V -k1,1 -k2,2n  > ${FOLDER}/${ROUND2}.SAM
...
rm  ${FOLDER}/${ROUND}_temp.bam ${FOLDER}/${ROUND}_readnonuniq.txt ${FOLDER}/${ROUND}_2.bam ${FOLDER}/${ROUND}_modified2.sam ${FOLDER}/${ROUND}_readcorrection.txt ${FOLDER}/${ROUND}_flag016.sam ${FOLDER}/${ROUND}_flag256.sam ${FOLDER}/${ROUND}_flag2562.sam ${FOLDER}/${ROUND}_flag0162.sam
